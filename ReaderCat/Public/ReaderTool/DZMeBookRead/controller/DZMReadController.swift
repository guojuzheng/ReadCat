...
class DZMReadController: DZMViewController,DZMReadMenuDelegate,DZMCoverControllerDelegate,UIPageViewControllerDelegate,UIPageViewControllerDataSource {
    ...
    func readMenuClickSubscribeButton(readMenu: DZMReadMenu, button: UIButton) {
        ...
        popupView.maximumWidth  = 130
        popupView.shouldShowItemSeparator = true
        ...
        popupView.items = [item1,item2,item3] as! [QMUIPopupMenuItem]
        
        popupView.didHideBlock = {(t:Bool?)in
            print("订阅菜单")
        }
        popupView.layout(withTargetView: button)
        popupView.showWith(animated: true)
    }
    
    /// 点击评论按钮
    func readMenuClickCommentButton(readMenu: DZMReadMenu, button: UIButton) {
        print("评论")
    }
